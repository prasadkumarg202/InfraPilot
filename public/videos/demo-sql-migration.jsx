...
const CAPTIONS=[
  'Four payment databases must move from SQL Server 2016 to a new Always On AG.',
  'Source, destination and change request — INFRsre plans the whole migration.',
  'Full backups are taken on the source and verified.',
  'Backups restore onto every AG replica — WITH NORECOVERY.',
  'Tail-log backup ships: the primary recovers, secondaries stay NORECOVERY.',
  'AG configured: automatic failover on the sync pair, an async remote replica for DR.',
  'Post-migration: compatibility level, CHECKDB, maintenance jobs — then the CR closes.',
  ''];
const PANEL=['none','req','backup','restore','logrec','agcfg','post','post'];
const TW=190,TH=88;
const SRV=[
  {name:'SQL-LEG-08', x:110, y:40, role:u=>['SOURCE · 2016',C.t3], steps:[
    [0,'online · 4 databases',C.t4],
    [2.10,'backing up 4 DBs…',C.gold,[2.10,2.85]],
    [2.85,'backups verified · 42.4 GB',C.green],
    [4.10,'tail-log backup…',C.gold,[4.10,4.30]],
    [4.30,'read-only · cutover done',C.blue]]},
  {name:'SQLAG-11A', x:10, y:266, role:u=>u<5.25?['REPLICA',C.t3]:['PRIMARY',C.gold], steps:[
    [0,'provisioned · SQL 2022',C.t4],
    [3.05,'restoring FULL…',C.gold,[3.05,3.40]],
    [3.40,'restored · NORECOVERY',C.blue],
    [4.35,'log — WITH RECOVERY',C.gold,[4.35,4.60]],
    [4.60,'online · recovered',C.green],
    [5.75,'sync · auto failover',C.green]]},
  {name:'SQLAG-11B', x:204, y:266, role:u=>u<5.25?['REPLICA',C.t3]:['SECONDARY',C.t3], steps:[
    [0,'provisioned · SQL 2022',C.t4],
    [3.25,'restoring FULL…',C.gold,[3.25,3.60]],
    [3.60,'restored · NORECOVERY',C.blue],
    [4.50,'log — NORECOVERY',C.gold,[4.50,4.75]],
    [4.75,'NORECOVERY · logs',C.blue],
    [5.75,'sync · auto failover',C.green]]},
  {name:'SQLAG-11C', x:452, y:266, role:u=>u<5.25?['REPLICA',C.t3]:['REMOTE · ASYNC',C.blue], steps:[
    [0,'provisioned · SQL 2022',C.t4],
    [3.45,'restoring FULL…',C.gold,[3.45,3.85]],
    [3.85,'restored · NORECOVERY',C.blue],
    [4.60,'log — NORECOVERY',C.gold,[4.60,4.85]],
    [4.85,'NORECOVERY · logs',C.blue],
...
function Panel({kind,t,u}){
  ...
  if(kind==='req'){ const rows=[
      ['Source','SQL-LEG-08 · SQL Server 2016',0.12],['Destination','SQLAG-11 · 3-replica AG · 2022',0.26],
      ['Change request','CHG0051204 · CAB approved',0.40],['Cutover window','Sat 01:00–05:00 IST',0.54]];
  ...
  if(kind==='backup') return <SeqList title="Full backups · SQL-LEG-08" meta="COPY_ONLY off" u={u} rows={[
      ['PayCore',2.25,'18.4 GB'],['PayAudit',2.42,'9.1 GB'],['PayArchive',2.60,'12.6 GB'],['PayRef',2.75,'2.3 GB']]}
      note="verified with RESTORE VERIFYONLY · checksums pass" noteAt={2.85}/>;
  if(kind==='restore') return <SeqList title="Restore FULL on AG replicas" meta="WITH NORECOVERY" u={u} rows={[
      ['Copy backup set to replicas',3.05],['Restore on SQLAG-11A',3.40],['Restore on SQLAG-11B',3.60],['Restore on SQLAG-11C',3.85]]}
      note="4 databases × 3 replicas · parallel restore" noteAt={3.9}/>;
  if(kind==='logrec') return <SeqList title="Tail-log backup & recovery" meta="cutover" u={u} rows={[
      ['Tail-log backup on source',4.30],['Restore log on 11A — WITH RECOVERY',4.60],
      ['Restore log on 11B — NORECOVERY',4.75],['Restore log on 11C — NORECOVERY',4.85]]}
      note="source frozen read-only · downtime clock: 4m 20s" noteAt={4.9}/>;
  if(kind==='agcfg') return <SeqList title="Always On configuration" meta="AG-PAY11" u={u} rows={[
      ['Create availability group AG-PAY11',5.18],['Join replicas 11B, 11C',5.38],
      ['11A + 11B synchronous · automatic failover',5.58],['11C remote replica — asynchronous',5.76],
      ['Add 4 databases — SYNCHRONIZED',5.92]]}
      note="listener ag-pay11.corp:1433 · DR via async remote" noteAt={5.96}/>;
  /* post */
  return <SeqList title="Post-migration steps" meta="RB-3120" u={u} rows={[
      ['Compatibility level 130 → 160',6.20],['DBCC CHECKDB — 4/4 clean',6.45],['Maintenance jobs configured',6.70,'backup · index · integrity']]}
      note="ServiceNow CHG0051204 → Closed · evidence attached" noteAt={6.88} badge="CR CLOSED" badgeAt={6.82}/>;
}
function Tile({sv,u}){
...
function Demo(){
  ...
  return (
    <div data-screen-label={'SQL migration demo · t='+Math.floor(gt)+'s'}
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,height:50,display:'flex',alignItems:'center',padding:'0 36px',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.1em',color:C.t4,whiteSpace:'nowrap'}}>PRODUCT DEMO · DATABASE MIGRATION FACTORY</span>
      </div>
          ...
          <div style={{display:'flex',gap:14,padding:14,height:'calc(100% - 72px)',boxSizing:'border-box'}}>
            ...
            <div style={{flex:1,position:'relative'}}>
              <div style={{position:'relative',height:410,marginTop:4}}>
                <div style={{position:'absolute',left:0,top:16,fontFamily:F.d,fontWeight:800,fontSize:9.5,letterSpacing:'.08em',color:C.t4,whiteSpace:'nowrap'}}>SOURCE · SQL SERVER 2016</div>
                <div style={{position:'absolute',left:0,top:214,fontFamily:F.d,fontWeight:800,fontSize:9.5,letterSpacing:'.08em',color:C.t4,whiteSpace:'nowrap'}}>DESTINATION · ALWAYS ON AG · SQL SERVER 2022 {u>5.15?'· AG-PAY11':''}</div>
      ...
      {titleO>0 && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          ...
          <div style={{fontFamily:F.d,fontWeight:800,fontSize:58,color:'#fff',letterSpacing:'-0.01em',whiteSpace:'nowrap',transform:'translateY('+(14*(1-titleO))+'px)'}}>Database migration factory</div>
          <div style={{fontFamily:F.b,fontSize:17,color:C.t2,marginTop:12,whiteSpace:'nowrap'}}>SQL Server 2016 → 2022 Always On, end to end</div>
        </div>)}
      {scrim>0 && (
        <div style={{position:'absolute',inset:0,background:'rgba(12,20,36,'+(0.88*scrim)+')',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:26}}>
          <div style={{display:'flex',gap:18}}>
            {[['4','databases migrated',C.t1],['4m 20s','cutover downtime',C.green],['CHG0051204','closed with evidence',C.gold]].map((d,i)=>(
