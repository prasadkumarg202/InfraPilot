/* INFRsre demo film — Incident self-healing. Scene components for animations-v2 SceneStage. */
...
const FEED = [
  ...
  {ph:1,at:0.22,c:C.red,  t:'02:41:07',m:'P1 · SQL Server service stopped on SQL-PRD-14',alert:true},
  {ph:2,at:0.22,c:C.blue, t:'02:41:31',m:'Incident INC0048122 created in ServiceNow — priority P1'},
  {ph:3,at:0.12,c:C.gold, t:'02:42:02',m:'Runbook RB-1108 invoked — start SQL Server service'},
  {ph:4,at:0.30,c:C.green,t:'02:44:19',m:'Service running — 14 databases online, INC0048122 closed'},
];
const CAPTIONS = [
  'The INFRsre operations console, watching 40,000 nodes. A P1 is about to happen.',
  '02:41:07 — the MSSQLSERVER service stops unexpectedly. INFRsre sees it instantly.',
  'An incident is created in ServiceNow automatically — INC0048122, priority P1.',
  'Runbook RB-1108 executes: the service is started and verified. Nobody is paged.',
  ...
  return (
    <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'12px 16px 8px',position:'relative'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline'}}>
        <span style={{fontFamily:F.d,fontWeight:700,fontSize:13,color:C.t2}}>SQL-PRD-14 · connection errors</span>
...
function Detail({kind,t,pulseT}){
  ...
  if(kind==='alert') return (
    <div style={{background:C.card,border:'1px solid rgba(241,125,119,.45)',borderRadius:10,padding:'16px 18px',height:'100%',boxSizing:'border-box'}}>
      ...
      <div style={{fontFamily:F.d,fontWeight:700,fontSize:17,color:C.t1,marginBottom:6}}>SQL Server service stopped on SQL-PRD-14</div>
      <div style={{fontFamily:F.b,fontSize:12.5,lineHeight:1.5,color:C.t3}}>MSSQLSERVER terminated unexpectedly · payment settlement cluster · Mumbai region. 14 databases offline — settlement API fails over in ~9 minutes if not restored.</div>
    </div>);
  if(kind==='incident'){ const rows=[
      ['Number','INC0048122',0.06],['Priority','P1 — Critical',0.18],
      ['Configuration item','SQL-PRD-14 · SQL Server 2022',0.30],['State','In Progress — automated',0.42],
  ...
  if(kind==='runbook'){ const steps=[
      ['Validate state — MSSQLSERVER stopped','02:42:04',0.10],['Check event log & disk — fault is clean','02:42:19',0.32],
      ['Start MSSQLSERVER service','02:42:47',0.54],['Verify 14 databases online · health checks','02:43:31',0.76]];
    return (
      <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'14px 18px',height:'100%',boxSizing:'border-box'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
          <span style={{fontFamily:F.d,fontWeight:700,fontSize:13,color:C.t1}}>Runbook RB-1108 · start SQL Server service</span>
  ...
  /* valid */
  const rows=[['MSSQLSERVER service','running',0.10],['Databases online','14/14',0.30],['Connection errors','0 /s · baseline',0.50]];
  ...
  return (
    <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'16px 18px',height:'100%',boxSizing:'border-box',position:'relative'}}>
      ...
      <div style={{fontFamily:F.m,fontSize:11,color:C.t4,marginTop:8,opacity:M.in(t,0.78,0.92),whiteSpace:'nowrap'}}>ServiceNow INC0048122 → Closed · resolution notes + audit trail attached</div>
    </div>);
}
  ...
  return (
    <div data-screen-label={'Self-healing demo · t='+Math.floor(gt)+'s'}
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,height:50,display:'flex',alignItems:'center',padding:'0 36px',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.1em',color:C.t4,whiteSpace:'nowrap'}}>PRODUCT DEMO · INCIDENT SELF-HEALING</span>
      </div>
      ...
      {titleO>0 && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          ...
          <div style={{fontFamily:F.d,fontWeight:800,fontSize:58,color:'#fff',letterSpacing:'-0.01em',whiteSpace:'nowrap',transform:'translateY('+(14*(1-titleO))+'px)'}}>Incident self-healing</div>
