...
const CAPTIONS=[
  'A March security patch is due across the PostgreSQL estate.',
  'Enter the change request — INFRsre pulls the affected servers from ServiceNow.',
  'Pre-checks run everywhere: backups, disk, replication lag, quorum, rollback snapshots.',
  'Topology detected — standalone, streaming replication, Patroni cluster. Each gets its own strategy.',
  'Streaming replication: patch the replicas first, then switchover, then the old primary.',
  'Patroni cluster: standby first, a controlled switchover, then the previous leader.',
  'The standalone server completes the wave — CHG0044521 is updated and closed.',
  ''];
const PANEL=['none','cr','pre','topo','agseq','fciseq','done','done'];
/* servers: patch windows in global u (phase+progress) */
const SERVERS=[
  {name:'PG-REPL-01A', grp:'ag',  slot:0, patch:[4.75,4.97]},
  {name:'PG-REPL-01B', grp:'ag',  slot:1, patch:[4.05,4.35]},
  {name:'PG-REPL-01C', grp:'ag',  slot:2, patch:[4.20,4.50]},
  {name:'PG-HA-02A',grp:'fci', slot:0, patch:[5.65,5.95]},
  {name:'PG-HA-02B',grp:'fci', slot:1, patch:[5.05,5.35]},
  {name:'PG-APP-03',grp:'sa',  slot:0, patch:[6.10,6.50]}];
const GROUP_Y={ag:30,fci:170,sa:310};
const GROUP_LABEL={ag:'STREAMING REPLICATION · PG-REPL-01',fci:'PATRONI CLUSTER · PG-HA-02',sa:'STANDALONE'};
const TW=190,TH=88,GX=206;
function roleAt(name,u){
  if(name==='PG-REPL-01A') return u<4.63?['PRIMARY',C.gold]:['REPLICA',C.t3];
  if(name==='PG-REPL-01B') return u<4.63?['REPLICA',C.t3]:['PRIMARY',C.gold];
  if(name==='PG-REPL-01C') return ['REPLICA',C.t3];
  if(name==='PG-HA-02A') return u<5.53?['LEADER',C.gold]:['STANDBY',C.t3];
  if(name==='PG-HA-02B') return u<5.53?['STANDBY',C.t3]:['LEADER',C.gold];
...
function Panel({kind,t,u}){
  ...
  if(kind==='cr'){ const rows=[
      ['Change request','CHG0044521',0.15],['Title','PostgreSQL 16 — security minor release 16.4',0.28],
  ...
  if(kind==='pre'){ const rows=[
      ['Full backups verified — 6/6',0.12],['Disk space & WAL headroom',0.26],['Replication lag < 5s everywhere',0.40],
      ['Patroni quorum healthy',0.54],['No pending reboots',0.68],['Rollback snapshots armed',0.82]];
    return (
      <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'14px 18px',height:'100%',boxSizing:'border-box'}}>
  ...
  if(kind==='topo'){ const rows=[
      ['Streaming replication','3 nodes — replicas first, then switchover, then old primary',C.gold,0.15],
      ['Patroni cluster','2 nodes — standby first, switchover, previous leader',C.blue,0.40],
  ...
  if(kind==='agseq') return <SeqList title="Streaming repl · PG-REPL-01" meta="RB-2310" u={u} rows={[
    ['Patch replica PG-REPL-01B',4.35],['Patch replica PG-REPL-01C',4.50],
    ['Switchover primary → 01B',4.72],['Patch former primary 01A',4.97]]}/>;
  if(kind==='fciseq') return <SeqList title="Patroni cluster · PG-HA-02" meta="RB-2311" u={u} rows={[
    ['Patch standby node 02B',5.35],['Controlled switchover 02A → 02B',5.62],['Patch previous leader 02A',5.95]]}/>;
  /* done */
  ...
  return (
    <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'14px 18px',height:'100%',boxSizing:'border-box',position:'relative'}}>
      ...
      {[['All 6 servers patched — 16.4',6.50],['Replication synchronized',6.60],['Validation checks 24/24 pass',6.68]].map((r,i)=>{
      ...
      <div style={{fontFamily:F.m,fontSize:11,color:C.t4,marginTop:8,opacity:M.in(u,6.8,6.92),whiteSpace:'nowrap'}}>ServiceNow CHG0044521 → Closed · evidence attached</div>
  ...
  return (
    <div style={{position:'absolute',left:pos.x,top:pos.y,width:TW,height:TH,opacity:appear,
      ...
      <div style={{marginTop:8}}>
        {done ? <span style={{fontFamily:F.m,fontSize:11,color:C.green,whiteSpace:'nowrap'}}>patched ✓ · 16.4</span>
        : patching ? (
          <div>
            <div style={{fontFamily:F.m,fontSize:10.5,color:C.gold,whiteSpace:'nowrap',marginBottom:4}}>patching 16.4… {Math.round(prog*100)}%</div>
  ...
  return (
    <div data-screen-label={'Patch orchestration demo · t='+Math.floor(gt)+'s'}
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,height:50,display:'flex',alignItems:'center',padding:'0 36px',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.1em',color:C.t4,whiteSpace:'nowrap'}}>PRODUCT DEMO · POSTGRESQL PATCHING</span>
      </div>
          ...
          <div style={{display:'flex',gap:14,padding:14,height:'calc(100% - 72px)',boxSizing:'border-box'}}>
            ...
            <div style={{flex:1,position:'relative'}}>
              <div style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.08em',color:C.t4,margin:'2px 0 8px',whiteSpace:'nowrap'}}>
                PATCH TARGETS {u>1.4?'· CHG0044521':''}</div>
              <div style={{position:'relative',height:410}}>
                ...
                <FailoverArrow u={u} from={{x:agA.slot*GX,y:GROUP_Y.ag}} to={{x:agB.slot*GX,y:GROUP_Y.ag}} win={[4.55,4.72]} label="SWITCHOVER"/>
                <FailoverArrow u={u} from={{x:fciA.slot*GX,y:GROUP_Y.fci}} to={{x:fciB.slot*GX,y:GROUP_Y.fci}} win={[5.45,5.62]} label="SWITCHOVER"/>
              </div>
            </div>
          </div>
      ...
      {titleO>0 && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          ...
          <div style={{fontFamily:F.d,fontWeight:800,fontSize:58,color:'#fff',letterSpacing:'-0.01em',whiteSpace:'nowrap',transform:'translateY('+(14*(1-titleO))+'px)'}}>PostgreSQL patching</div>
          <div style={{fontFamily:F.b,fontSize:17,color:C.t2,marginTop:12,whiteSpace:'nowrap'}}>One change request. Six PostgreSQL servers. Zero downtime.</div>
        </div>)}
      {scrim>0 && (
        <div style={{position:'absolute',inset:0,background:'rgba(12,20,36,'+(0.88*scrim)+')',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:26}}>
          <div style={{display:'flex',gap:18}}>
            {[['6','servers patched',C.t1],['0','downtime',C.green],['CHG0044521','closed with evidence',C.gold]].map((d,i)=>(
