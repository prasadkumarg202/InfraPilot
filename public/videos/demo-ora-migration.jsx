...
const CAPTIONS=[
  'Four payment databases must move from Oracle 12c to a new 19c Data Guard estate.',
  'Source, destination and change request — INFRsre plans the whole migration.',
  'RMAN full backups snapshot the source — verified and cataloged.',
  'RMAN restores onto every node — standbys in managed recovery.',
  'Redo catch-up ships — the new primary opens, standbys keep applying.',
  'Data Guard configured: SYNC pair with fast-start failover, ASYNC remote for DR.',
  'Post-upgrade: stats gather, datapatch, maintenance jobs — then the CR closes.',
  ''];
const PANEL=['none','req','backup','restore','logrec','agcfg','post','post'];
const TW=190,TH=88;
const SRV=[
  {name:'ORA-LEG-02', x:110, y:40, role:u=>['SOURCE · ORA 12c',C.t3], steps:[
    [0,'online · 4 databases',C.t4],
    [2.10,'RMAN backup 4 DBs…',C.gold,[2.10,2.85]],
    [2.85,'backups verified · 42.4 GB',C.green],
    [4.10,'final redo ship…',C.gold,[4.10,4.30]],
    [4.30,'read-only · cutover done',C.blue]]},
  {name:'ORA-19A', x:10, y:266, role:u=>u<5.25?['REPLICA',C.t3]:['PRIMARY',C.gold], steps:[
    [0,'provisioned · ORA 19c',C.t4],
    [3.05,'RMAN restore…',C.gold,[3.05,3.40]],
    [3.40,'restored · standby',C.blue],
    [4.35,'open — primary',C.gold,[4.35,4.60]],
    [4.60,'online · open',C.green],
    [5.75,'sync · auto failover',C.green]]},
  {name:'ORA-19B', x:204, y:266, role:u=>u<5.25?['REPLICA',C.t3]:['SECONDARY',C.t3], steps:[
    [0,'provisioned · ORA 19c',C.t4],
    [3.25,'RMAN restore…',C.gold,[3.25,3.60]],
    [3.60,'restored · standby',C.blue],
    [4.50,'applying redo',C.gold,[4.50,4.75]],
    [4.75,'standby · redo apply',C.blue],
    [5.75,'sync · auto failover',C.green]]},
  {name:'ORA-19C', x:452, y:266, role:u=>u<5.25?['REPLICA',C.t3]:['REMOTE · ASYNC',C.blue], steps:[
    [0,'provisioned · ORA 19c',C.t4],
    [3.45,'RMAN restore…',C.gold,[3.45,3.85]],
    [3.85,'restored · standby',C.blue],
    [4.60,'applying redo',C.gold,[4.60,4.85]],
    [4.85,'standby · redo apply',C.blue],
...
function Panel({kind,t,u}){
  ...
  if(kind==='req'){ const rows=[
      ['Source','ORA-LEG-02 · Oracle 12c',0.12],['Destination','ORA-19 · 3-node Data Guard · 19c',0.26],
      ['Change request','CHG0053017 · CAB approved',0.40],['Cutover window','Sat 01:00–05:00 IST',0.54]];
  ...
  if(kind==='backup') return <SeqList title="RMAN backups · ORA-LEG-02" meta="cataloged" u={u} rows={[
      ['PayCore',2.25,'18.4 GB'],['PayAudit',2.42,'9.1 GB'],['PayArchive',2.60,'12.6 GB'],['PayRef',2.75,'2.3 GB']]}
      note="verified — archivelog shipping continuous" noteAt={2.85}/>;
  if(kind==='restore') return <SeqList title="RMAN restore on all nodes" meta="standby" u={u} rows={[
      ['Copy backup set to replicas',3.05],['Restore on ORA-19A',3.40],['Restore on ORA-19B',3.60],['Restore on ORA-19C',3.85]]}
      note="4 databases × 3 replicas · parallel restore" noteAt={3.9}/>;
  if(kind==='logrec') return <SeqList title="Redo catch-up & open" meta="cutover" u={u} rows={[
      ['Final redo shipped & applied',4.30],['Open 19A — new primary · upgrade run',4.60],
      ['19B applying redo from primary',4.75],['19C applying — async remote',4.85]]}
      note="source frozen read-only · downtime clock: 4m 20s" noteAt={4.9}/>;
  if(kind==='agcfg') return <SeqList title="Data Guard configuration" meta="ORA-PAY19" u={u} rows={[
      ['Create broker config · ORA-PAY19',5.18],['Enable standbys 19B, 19C',5.38],
      ['19A + 19B SYNC · fast-start failover',5.58],['19C remote standby — ASYNC',5.76],
      ['All 4 databases — in sync',5.92]]}
      note="listener ora-pay19.corp:1521 · DR via async remote" noteAt={5.96}/>;
  /* post */
  return <SeqList title="Post-upgrade steps" meta="RB-3120" u={u} rows={[
      ['Gather stats · fixed objects',6.20],['datapatch & utlrp — clean',6.45],['Maintenance jobs configured',6.70,'backup · index · integrity']]}
      note="ServiceNow CHG0053017 → Closed · evidence attached" noteAt={6.88} badge="CR CLOSED" badgeAt={6.82}/>;
}
function Tile({sv,u}){
...
function Demo(){
  ...
  return (
    <div data-screen-label={'Oracle upgrade demo · t='+Math.floor(gt)+'s'}
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,height:50,display:'flex',alignItems:'center',padding:'0 36px',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.1em',color:C.t4,whiteSpace:'nowrap'}}>PRODUCT DEMO · ORACLE UPGRADE</span>
      </div>
          ...
          <div style={{display:'flex',gap:14,padding:14,height:'calc(100% - 72px)',boxSizing:'border-box'}}>
            ...
            <div style={{flex:1,position:'relative'}}>
              <div style={{position:'relative',height:410,marginTop:4}}>
                <div style={{position:'absolute',left:0,top:16,fontFamily:F.d,fontWeight:800,fontSize:9.5,letterSpacing:'.08em',color:C.t4,whiteSpace:'nowrap'}}>SOURCE · ORACLE 12C</div>
                <div style={{position:'absolute',left:0,top:214,fontFamily:F.d,fontWeight:800,fontSize:9.5,letterSpacing:'.08em',color:C.t4,whiteSpace:'nowrap'}}>DESTINATION · DATA GUARD · ORACLE 19C {u>5.15?'· ORA-PAY19':''}</div>
      ...
      {titleO>0 && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          ...
          <div style={{fontFamily:F.d,fontWeight:800,fontSize:58,color:'#fff',letterSpacing:'-0.01em',whiteSpace:'nowrap',transform:'translateY('+(14*(1-titleO))+'px)'}}>Oracle upgrade factory</div>
          <div style={{fontFamily:F.b,fontSize:17,color:C.t2,marginTop:12,whiteSpace:'nowrap'}}>Oracle 12c → 19c, end to end — near-zero downtime</div>
        </div>)}
      {scrim>0 && (
        <div style={{position:'absolute',inset:0,background:'rgba(12,20,36,'+(0.88*scrim)+')',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:26}}>
          <div style={{display:'flex',gap:18}}>
            {[['4','databases upgraded',C.t1],['4m 20s','cutover downtime',C.green],['CHG0053017','closed with evidence',C.gold]].map((d,i)=>(
