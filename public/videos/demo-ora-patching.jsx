...
const CAPTIONS=[
  'A March security patch is due across the Oracle estate.',
  'Enter the change request — INFRsre pulls the affected servers from ServiceNow.',
  'Pre-checks run everywhere: backups, disk, Data Guard lag, cluster health, rollback snapshots.',
  'Topology detected — standalone, Data Guard, RAC cluster. Each gets its own strategy.',
  'Data Guard: patch the standbys first, then switchover, then the old primary.',
  'RAC: rolling patch — services relocate, one instance at a time, no outage.',
  'The standalone server completes the wave — CHG0045108 is updated and closed.',
  ''];
const PANEL=['none','cr','pre','topo','agseq','fciseq','done','done'];
/* servers: patch windows in global u (phase+progress) */
const SERVERS=[
  {name:'ORA-DG-01A', grp:'ag',  slot:0, patch:[4.75,4.97]},
  {name:'ORA-DG-01B', grp:'ag',  slot:1, patch:[4.05,4.35]},
  {name:'ORA-DG-01C', grp:'ag',  slot:2, patch:[4.20,4.50]},
  {name:'ORA-RAC-02A',grp:'fci', slot:0, patch:[5.65,5.95]},
  {name:'ORA-RAC-02B',grp:'fci', slot:1, patch:[5.05,5.35]},
  {name:'ORA-APP-03',grp:'sa',  slot:0, patch:[6.10,6.50]}];
const GROUP_Y={ag:30,fci:170,sa:310};
const GROUP_LABEL={ag:'DATA GUARD · ORA-DG-01',fci:'RAC CLUSTER · ORA-RAC-02',sa:'STANDALONE'};
const TW=190,TH=88,GX=206;
function roleAt(name,u){
  if(name==='ORA-DG-01A') return u<4.63?['PRIMARY',C.gold]:['STANDBY',C.t3];
  if(name==='ORA-DG-01B') return u<4.63?['STANDBY',C.t3]:['PRIMARY',C.gold];
  if(name==='ORA-DG-01C') return ['STANDBY',C.t3];
  if(name==='ORA-RAC-02A') return u<5.53?['INST 1',C.gold]:['INST 2',C.t3];
  if(name==='ORA-RAC-02B') return u<5.53?['INST 2',C.t3]:['INST 1',C.gold];
...
function Panel({kind,t,u}){
  ...
  if(kind==='cr'){ const rows=[
      ['Change request','CHG0045108',0.15],['Title','Oracle 19c — quarterly RU (19.24 DBRU)',0.28],
  ...
  if(kind==='pre'){ const rows=[
      ['Full backups verified — 6/6',0.12],['Disk space & WAL headroom',0.26],['Data Guard lag < 5s · sync healthy',0.40],
      ['RAC cluster & voting disks healthy',0.54],['No pending reboots',0.68],['Rollback snapshots armed',0.82]];
    return (
      <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'14px 18px',height:'100%',boxSizing:'border-box'}}>
  ...
  if(kind==='topo'){ const rows=[
      ['Data Guard','3 nodes — standbys first, then switchover, then old primary',C.gold,0.15],
      ['RAC cluster','2 nodes — rolling: one instance at a time, services relocate',C.blue,0.40],
  ...
  if(kind==='agseq') return <SeqList title="Data Guard · ORA-DG-01" meta="RB-2310" u={u} rows={[
    ['Patch standby ORA-DG-01B',4.35],['Patch standby ORA-DG-01C',4.50],
    ['DG switchover → 01B',4.72],['Patch former primary 01A',4.97]]}/>;
  if(kind==='fciseq') return <SeqList title="RAC rolling · ORA-RAC-02" meta="RB-2311" u={u} rows={[
    ['Relocate services · patch inst 2',5.35],['Rolling — services back on 02B',5.62],['Drain & patch instance 02A',5.95]]}/>;
  /* done */
  ...
  return (
    <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'14px 18px',height:'100%',boxSizing:'border-box',position:'relative'}}>
      ...
      {[['All 6 servers patched — 19.24 RU',6.50],['Replication synchronized',6.60],['Validation checks 24/24 pass',6.68]].map((r,i)=>{
      ...
      <div style={{fontFamily:F.m,fontSize:11,color:C.t4,marginTop:8,opacity:M.in(u,6.8,6.92),whiteSpace:'nowrap'}}>ServiceNow CHG0045108 → Closed · evidence attached</div>
  ...
  return (
    <div style={{position:'absolute',left:pos.x,top:pos.y,width:TW,height:TH,opacity:appear,
      ...
      <div style={{marginTop:8}}>
        {done ? <span style={{fontFamily:F.m,fontSize:11,color:C.green,whiteSpace:'nowrap'}}>patched ✓ · 19.24</span>
        : patching ? (
          <div>
            <div style={{fontFamily:F.m,fontSize:10.5,color:C.gold,whiteSpace:'nowrap',marginBottom:4}}>applying RU… {Math.round(prog*100)}%</div>
  ...
  return (
    <div data-screen-label={'Patch orchestration demo · t='+Math.floor(gt)+'s'}
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,height:50,display:'flex',alignItems:'center',padding:'0 36px',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.1em',color:C.t4,whiteSpace:'nowrap'}}>PRODUCT DEMO · ORACLE PATCHING</span>
      </div>
          ...
          <div style={{display:'flex',gap:14,padding:14,height:'calc(100% - 72px)',boxSizing:'border-box'}}>
            ...
            <div style={{flex:1,position:'relative'}}>
              <div style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.08em',color:C.t4,margin:'2px 0 8px',whiteSpace:'nowrap'}}>
                PATCH TARGETS {u>1.4?'· CHG0045108':''}</div>
      ...
      {titleO>0 && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          ...
          <div style={{fontFamily:F.d,fontWeight:800,fontSize:58,color:'#fff',letterSpacing:'-0.01em',whiteSpace:'nowrap',transform:'translateY('+(14*(1-titleO))+'px)'}}>Oracle patching</div>
          <div style={{fontFamily:F.b,fontSize:17,color:C.t2,marginTop:12,whiteSpace:'nowrap'}}>One change request. Six Oracle servers. Zero downtime.</div>
        </div>)}
      {scrim>0 && (
        <div style={{position:'absolute',inset:0,background:'rgba(12,20,36,'+(0.88*scrim)+')',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:26}}>
          <div style={{display:'flex',gap:18}}>
            {[['6','servers patched',C.t1],['0','downtime',C.green],['CHG0045108','closed with evidence',C.gold]].map((d,i)=>(
