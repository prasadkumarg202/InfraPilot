...
const CAPTIONS=[
  'A new production Oracle server, built hands-free from an intake request.',
  'Intake RITM0069884: OS, Oracle version, compute — and the full mount layout.',
  'The VM is created first — six disks attached, powered on, enrolled in IdM.',
  'Filesystems created and mounted: OS, ORACLE_HOME, data, redo, FRA, temp.',
  'Pre-build: oracle user & groups, firewall, huge pages, kernel params, limits.',
  'Oracle 19c installs silently — DBCA creates the database, redo on /u03.',
  'Post-build: SGA/PGA targets, archivelog, RMAN and company-policy hardening.',
...
const TW=190;
const VM={name:'ORANEW-01', x:206, y:34, steps:[
  ...
  [5.10,'installing 19c…',C.gold,[5.10,5.75]],
  [5.80,'19c + RU · /u01',C.green],
...
const DRIVES=[
  ...
  {l:'/u01',label:'OraHome',size:'100 GB',fmt:'xfs'},
  {l:'/u02',label:'OraData',size:'500 GB',fmt:'xfs'},
  {l:'/u03',label:'Redo',size:'250 GB',fmt:'xfs'},
  {l:'/u04',label:'FRA',size:'1 TB',fmt:'xfs'},
  {l:'/u05',label:'Temp',size:'200 GB',fmt:'xfs'}];
function Check({e}){
  return (
...
function Panel({kind,t,u}){
  ...
  if(kind==='intake'){ const rows=[
      ['Intake request','RITM0069884 · approved',0.10],['Operating system','RHEL 9',0.22],
      ['Oracle','19c EE + 19.24 RU',0.34],['Compute','8 vCPU · 64 GB RAM',0.46],
      ['ORACLE_HOME','/u01/app/oracle',0.58]];
    return (
      <div style={{background:C.card,border:'1px solid '+C.line,borderRadius:10,padding:'14px 18px',height:'100%',boxSizing:'border-box'}}>
        ...
        <div style={{marginTop:8,paddingTop:8,borderTop:'1px solid '+C.line,opacity:M.in(t,0.66,0.80)}}>
          ...
          <div style={{fontFamily:F.m,fontSize:10.5,color:C.t2,lineHeight:1.6,whiteSpace:'nowrap'}}>/ 150 · /u01 home 100 · /u02 data 500</div>
          <div style={{fontFamily:F.m,fontSize:10.5,color:C.t2,lineHeight:1.6,whiteSpace:'nowrap'}}>/u03 redo 250 · /u04 FRA 1 TB · /u05 temp 200</div>
        </div>
      </div>);
  }
  if(kind==='vm') return <SeqList title="Create VM · ORANEW-01" meta="vSphere" u={u} rows={[
  ...
  if(kind==='drives') return <SeqList title="Create & mount filesystems" meta="6 volumes" u={u} rows={[
      ['mkfs.xfs on all data volumes',3.45],['Mount & label (u01…u05)',3.70],
      ['fstab entries · mount options',3.85],['Verify permissions · oracle:oinstall',3.95]]}
  ...
  if(kind==='pre') return <SeqList title="Pre-build steps" meta="RB-4102" u={u} rows={[
      ['Service accounts from vault',4.20],['Firewall — 1521',4.40],
      ['tuned — throughput-performance',4.55],['Huge pages configured',4.70],['kernel params & limits applied',4.85]]}/>;
  if(kind==='install') return <SeqList title="Install Oracle 19c" meta="silent" u={u} rows={[
      ['runInstaller — silent · /u01',5.15],['DBCA — database on /u02',5.45],
      ['Apply 19.24 RU · datapatch',5.70],['Services running · instance up',5.90]]}
      note="data /u02 · redo /u03 · temp /u05 · FRA /u04" noteAt={5.92}/>;
  if(kind==='post') return <SeqList title="Post-build configuration" meta="policy v3.2" u={u} rows={[
      ['SGA 40 GB · PGA 12 GB',6.20],['archivelog on · FRA quota',6.40],
      ['RMAN backups → /u04',6.60],['TDE & network policy',6.78],['Maintenance jobs configured',6.92]]}/>;
  /* health */
  return <SeqList title="Health checks & handover" meta="24 checks" u={u} rows={[
      ['Connectivity & authentication',7.15],['RMAN test backup verified',7.35],
      ['Perf counters — all green',7.55],['Monitoring & alerting enrolled',7.70]]}
      note="ServiceNow RITM0069884 → Closed · docs attached" noteAt={7.88} badge="BUILD COMPLETE" badgeAt={7.82}/>;
}
function VmTile({u}){
  let cur=VM.steps[0];
  ...
  return (
    <div style={{position:'absolute',left:VM.x,top:VM.y,width:TW,height:92,opacity:vis,transform:'scale('+M.pop(vis)+')',
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:6}}>
        <span style={{fontFamily:F.m,fontSize:12,color:C.t1,whiteSpace:'nowrap'}}>ORANEW-01</span>
...
function Demo(){
  ...
  return (
    <div data-screen-label={'Oracle build demo · t='+Math.floor(gt)+'s'}
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,height:50,display:'flex',alignItems:'center',padding:'0 36px',justifyContent:'space-between'}}>
        ...
        <span style={{fontFamily:F.d,fontWeight:800,fontSize:10.5,letterSpacing:'.1em',color:C.t4,whiteSpace:'nowrap'}}>PRODUCT DEMO · ORACLE BUILD AUTOMATION</span>
      </div>
      <div style={{position:'absolute',left:110,top:76,width:1060,height:526,
        ...
        <div style={{position:'absolute',inset:0,background:C.panel,border:'1px solid '+C.line2,borderRadius:12,overflow:'hidden',boxShadow:'0 24px 60px rgba(0,0,0,.45)'}}>
          ...
          <div style={{display:'flex',gap:14,padding:14,height:'calc(100% - 72px)',boxSizing:'border-box'}}>
            ...
            <div style={{flex:1,position:'relative'}}>
              <div style={{position:'relative',height:410,marginTop:4}}>
                <div style={{position:'absolute',left:0,top:12,fontFamily:F.d,fontWeight:800,fontSize:9.5,letterSpacing:'.08em',color:C.t4,whiteSpace:'nowrap'}}>NEW BUILD · ORANEW-01 {u>1.4?'· RITM0069884':''}</div>
                <VmTile u={u}/>
          ...
          <div style={{display:'flex',justifyContent:'space-between',padding:'8px 16px',borderTop:'1px solid '+C.line,fontFamily:F.m,fontSize:10.5,color:C.t4,whiteSpace:'nowrap'}}>
            <span style={{whiteSpace:'nowrap'}}>template: RHEL9-ORA-v1.8</span><span style={{whiteSpace:'nowrap'}}>policy: company baseline</span><span style={{whiteSpace:'nowrap'}}>audit: signing all actions</span>
          </div>
        </div>
      </div>
      ...
      {titleO>0 && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          ...
          <div style={{fontFamily:F.d,fontWeight:800,fontSize:58,color:'#fff',letterSpacing:'-0.01em',whiteSpace:'nowrap',transform:'translateY('+(14*(1-titleO))+'px)'}}>Oracle build automation</div>
      ...
      {scrim>0 && (
        <div style={{position:'absolute',inset:0,background:'rgba(12,20,36,'+(0.88*scrim)+')',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:26}}>
          <div style={{display:'flex',gap:18}}>
            {[['38 min','intake to production',C.t1],['0','manual steps',C.green],['RITM0069884','closed with docs',C.gold]].map((d,i)=>(
